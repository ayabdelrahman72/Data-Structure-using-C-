...
class arrayListType
{
    public:
        ...
        int seqSearch(const int &item,int startPos)const;
        //  Recursive Binary search
        //NOTE: we assume that the user will assign the values in ORDER
        int recBinarySearch(const int &item,int first,int last);
        ///Recursive sequential ordered search function
         ///NOTE: we assume that the user will assign the values in ACESNDING ORDER
           int  recSeqOrdSearch(const int &item,int startPos)const;///NEW
