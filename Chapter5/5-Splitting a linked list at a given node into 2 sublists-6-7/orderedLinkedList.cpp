...
void orderedLinkedList::divideAt(linkedListType &secondList,const int &item){
      nodeType *current;
      nodeType *trailCurrent;
      int sizeFirstList=1;
      current=first;
  ...
  void orderedLinkedList::uniqueInsert(const int &newItem){
      if(!Search(newItem)){
       insertFirst(newItem);
      }else
      cout<<"Failed to insert "<<newItem<< " because it already EXISTS in the list"<<endl;
