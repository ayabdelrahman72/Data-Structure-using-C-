...
const linkedQueueType&linkedQueueType::operator=(const linkedQueueType& other){
         destroyQueue();
        copyQueue(other);
        return *this;
   }
 ...
 linkedQueueType::linkedQueueType(const linkedQueueType &other){
     //here the difference between this code and the code in assignment operator overloading is i deleted destroyQueue() function
     //cause this is constructor so the queue is not initialized yet to destroy it here we initialize the queue with another queue
          queueFront=NULL;
          queueRear=NULL;
...
void linkedQueueType::copyQueue(const linkedQueueType &other){

        if(other.queueFront !=NULL){
        nodeType *temp=other.queueFront;
       while(temp!=NULL){
           addQueue(temp->entry);
            temp=temp->next;
        }
